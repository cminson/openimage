#!/bin/bash
#
# Developed by Fred Weinhaus 3/11/2008 .......... revised 9/27/2010
#
...
framedir="/home/httpd/vhosts/ezimba/httpdocs/wimages/picframecorners"
frameid=1
mattesize=0
mattecolor="cornsilk"
bordersize=0
...
trap "rm -f $tmpA $tmpB $tmp0A $tmp0B $tmp1A $tmp1B $tmp2A $tmp2B $tmp3A $tmp3B; exit 0" 0
trap "rm -f $tmpA $tmpB $tmp0A $tmp0B $tmp1A $tmp1B $tmp2A $tmp2B $tmp3A $tmp3B; exit 1" 1 2 3 15

# test infile
if convert -quiet -regard-warnings "$infile" +repage "$tmp0A"
	then
...
#	blur=`convert xc: -format "%[fx:$distance/2]" info:`
	blur=`convert xc: -format "%[fx:min($distance-1,2*$distance/3)]" info:`
	convert $tmp3A \
		\( $tmp2A -background black -shadow ${opacity}x${blur}+${distance}+${distance} \
		-gravity center -crop ${w3}x${h3}-${distance}-${distance} +repage \) \
