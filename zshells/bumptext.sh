...
/usr/bin/convert -quiet -regard-warnings "$infile" -alpha off +repage $colorization "$tmpA1" ||
...
if [ "$mode" = "label" -a "$pointsize" != "" -a "$size" = "" ]; then
	/usr/bin/convert -background black -fill white \
		-font $font -pointsize $pointsize -gravity center \
		${mode}:"$text" $tmpA2
	ww=`/usr/bin/convert $tmpA2 -ping -format "%w" info:`
	hh=`/usr/bin/convert $tmpA2 -ping -format "%h" info:`
...
grayval=`/usr/bin/convert $tmpA2[1x1+0+0] -format "%[fx:100*u.r]" info:`
...
/usr/bin/convert $tmpA1 \( $tmpA2 -trim +repage -transparent "gray($grayval%)" \) \
	-gravity "$gravity" -geometry "$offsets" -compose modulate \
	-set option:compose:args $intensity -composite $outfile
else
/usr/bin/convert $tmpA2 -trim +repage -transparent "gray($grayval%)" $tmpA2
