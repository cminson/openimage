#!/bin/bash
#
# Developed by Fred Weinhaus 6/12/2010 .......... 11/28/2011
#
...
# each cell center point across the input image. A progress meter is therefore 
# provided to the terminal. A speed-up is available via a -process function, 
# getColors. To obtain getColors, contact me. It requires IM 6.6.2-10 or 
# higher.
# 
...
# get colorspace
colorspace=`/usr/local/bin/convert $infile -ping -format "%[colorspace]" info:`
# set up for modulate
if [ "$bright" != "100" -a "$colorspace" = "CMYK" ]; then
	modulation="-colorspace rgb -modulate ${bright},100,100 -colorspace cmyk"
elif [ "$bright" != "100" -a "$colorspace" = "CMYKA" ]; then
	modulation="-colorspace rgba -modulate ${bright},100,100 -colorspace cmyka"
...
/usr/local/bin/convert -quiet -regard-warnings "$infile" $reduce $modulation +repage "$tmpA1" ||
...
if [ "$im_version" -ge "06060210" ]; then
	process_test=`/usr/local/bin/convert -list module | grep "getColors"`
fi
	
ww=`/usr/local/bin/convert $tmpA1 -ping -format "%w" info:`
hh=`/usr/local/bin/convert $tmpA1 -ping -format "%h" info:`
ww1=$(($ww-1))
hh1=$(($hh-1))
ww2=`/usr/local/bin/convert xc: -format "%[fx:$ww1+round($size/2)]" info:`
hh2=`/usr/local/bin/convert xc: -format "%[fx:$hh1+round($size/2)]" info:`
...
if [ "$thick" = "0" ]; then
	if [ "$im_version" -lt "06060210" -o "$process_test" != "getColors" ]; then
		echo ""
		echo "Progress:"
		echo ""
		/usr/local/bin/convert $tmpA1 \( -background black $tmpC \) \
			-alpha off -compose copy_opacity -composite -monitor \
			txt:- |\
			sed '1d; / 0) /d; s/:.* /,/;' |\
			/usr/local/bin/convert -size ${ww}x${hh} xc: -sparse-color voronoi '@-' \
			$outfile
		echo ""
	else
		/usr/local/bin/convert $tmpA1 \( -background black $tmpC \) \
			-alpha off -compose copy_opacity -composite $tmpA1
		/usr/local/bin/convert $tmpA1 -alpha on -process "getColors" null: > $tmpC
		/usr/local/bin/convert -size ${ww}x${hh} xc: -sparse-color voronoi "@$tmpC" \
			$outfile
	fi
	...
	if [ "$im_version" -lt "06060210" -o "$process_test" != "getColors" ]; then
		...
		/usr/local/bin/convert $tmpA1 \( -background black $tmpC \) \
			...
			txt:- |\
			sed '1d; / 0) /d; s/:.* /,/;' |\
			/usr/local/bin/convert -size ${ww}x${hh} xc: -sparse-color voronoi '@-' $tmpA2
		/usr/local/bin/convert -size ${ww}x${hh} xc: -sparse-color voronoi "@$tmpG" \
			-auto-level -morphology edge diamond:$thick -threshold 0 -negate $tmpA3
		/usr/local/bin/convert $tmpA2 $tmpA3 -alpha off -compose copy_opacity -composite \
		...
		/usr/local/bin/convert $tmpA1 \( -background black $tmpC \) \
			-alpha off -compose copy_opacity -composite $tmpA1
		/usr/local/bin/convert $tmpA1 -alpha on -process "getColors" null: > $tmpC
		/usr/local/bin/convert -size ${ww}x${hh} xc: -sparse-color voronoi "@$tmpC" $tmpA2
		/usr/local/bin/convert -size ${ww}x${hh} xc: -sparse-color voronoi "@$tmpG" \
			-auto-level -morphology edge diamond:$thick -threshold 0 -negate $tmpA3
		/usr/local/bin/convert $tmpA2 $tmpA3 -alpha off -compose copy_opacity -composite \
			-compose over -background $ecolor -flatten \
