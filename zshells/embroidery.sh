#!/bin/bash
#
# Developed by Fred Weinhaus 8/20/2015 .......... revised 9/24/2015
#
# ------------------------------------------------------------------------------
# 
# Licensing:
...
#                      default=8
# -p     pattern       weave pattern; choices are either 1 or 2; default=1
...
# -C     contrast      bevel sigmoidal-contrast amount; integer>=0; 
...
# 
# -p pattern ... PATTERN is the weave pattern. The choices are either 1 or 2.
# The default=1.
# 
...
# integers>=0. The default=0 (no added contrast)
# 
...
pattern=1			# weave pattern; 1 or 2
...
newseed=100			# pattern new seed value; integer>=0
	...
	while [ $# -gt 0 ]
	  ...
	  -h|-help)    # help information
			...
			-p)    # get pattern
				   ...
				   checkMinus "$1"
				   pattern=`expr "$1" : '\([1-2]\)'`
				   [ "$pattern" = "" ] && errMsg "--- PATTERN=$pattern MUST BE AN INTEGER VALUE OF 1 or 2 ---"
				   ;;
			-t)    # get thickness
				   shift  # to get the next parameter
				   # test if parameter starts with minus sign 
			...
			-C)    # get contrast
				   ...
				   checkMinus "$1"
				   contrast=`expr "$1" : '\([0-9]*\)'`
				   [ "$contrast" = "" ] && errMsg "--- CONTAST=$contrast MUST BE A NON-NEGATIVE INTEGER ---"
				   ;;
			-S)    # get spread
				   shift  # to get the next parameter
				   # test if parameter starts with minus sign 
...
if [ $spread -eq 0 ]; then
	sproc=""
	mproc=""
elif [ $spread -eq 100 ]; then
	sproc="-clone 0 -seed $newseed -spread $spread"
	mproc="-delete 0"
else
	sproc="-clone 0 -seed $newseed -spread $spread"
...
#echo "thick1=$thick1; thick2=$thick2; thick3=$thick3; fullthick=$fullthick coarseness=$coarseness;"
