...
if /usr/bin/convert -quiet -regard-warnings "$infile" -colorspace gray +repage "$tmpA"
	then
	: ' Do Nothing '
else
...
offset=`/usr/bin/convert xc: -format "%[fx:$offset*128/100]" info:`

# create lut
/usr/bin/convert -size 1x1 xc:$lcolor xc:$mcolor xc:$hcolor +append \
...
# save lut
if [ "$lutfile" != "" ]; then
	/usr/bin/convert $tmp0 -scale 256x20! $lutfile
fi

# apply lut
if [ "$outfile" != "" ]; then
	/usr/bin/convert $tmpA $tmp0 -clut $outfile
fi
